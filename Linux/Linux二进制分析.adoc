...
https://github.com/lotabout/write-a-C-interpreter[手把手教你写C解释器]


https://www.136.la/tech/show-90463.html



https://blog.csdn.net/Rong_Toa/article/details/109825818

https://blog.csdn.net/Rong_Toa/article/details/109825818#t1
...
我们通过因为.o文件也是ELF文件的一种，所以我么可以使用readelf -h 来查看.o文件的elf头数据
...
(gdb) p *section_table
$11 = {sh_name = 0, sh_type = 0, sh_flags = 0, sh_addr = 0, sh_offset = 0, sh_size = 0, sh_link = 0, sh_info = 0, sh_addralign = 0, sh_entsize = 0}
(gdb) n
492
(gdb) p *section_table
$12 = {sh_name = 0, sh_type = 0, sh_flags = 0, sh_addr = 0, sh_offset = 0, sh_size = 0, sh_link = 0, sh_info = 0, sh_addralign = 0, sh_entsize = 0}
(gdb) p section_string_table_index
$13 = 35
(gdb) p 38488 - 36184
$14 = 2304
(gdb) p 36 * 64
...
Position-Independent-Executable是Binutils,glibc和gcc的⼀个功能，能⽤来创建介于共享库和通常可执⾏代码之间的代码–能像共享库
⼀样可重分配地址的程序，这种程序必须连接到Scrt1.o。标准的可执⾏程序需要固定的地址，并且只有被装载到这个地址时，程序才能正
确执⾏。PIE能使程序像共享库⼀样在主存任何位置装载，这需要将程序编译成位置⽆关，并链接为ELF共享对象。
引⼊PIE的原因是让程序能装载在随机的地址，通常情况下，内核都在固定的地址运⾏，如果能改⽤位置⽆关，那攻击者就很难借助系统中
的可执⾏码实施攻击了。类似缓冲区溢出之类的攻击将⽆法实施。⽽且这种安全提升的代价很⼩
谈到PIE就不得不说说Pax和Grsec内核。这两个东西都是为了构建坚不可摧到安全系统⽽准备的。PaX是Linux内核安全增强补丁，它能在
两⽅⾯保证安全性，⼀是ASLR(Address Space Layout Randomization,地址空间分布随机化)，这是⼀种将所有数据装载到内存时都随
机化地址的⽅式，当使⽤PIE选项编译应⽤时，PaX能将应⽤的地址做随机加法；⼆是能提供不可执⾏的内存空间，这样就能使得攻击者放
⼊内存中的恶意代码不可执⾏。不过PaX官⽹上能⽀持的最新内核是2.6.27，已经是⼀年前的更新了。Grsec也时类似的内核补丁，更新较
为频繁能⽀持最新的2.6.32内核。这两种⽅式都能将内核完全位置⽆关，除了Grub和Glibc中⽆法位置⽆关的汇编码。
PIE最早由RedHat的⼈实现，他在连接起上增加了-pie选项，这样使⽤-fPIE编译的对象就能通过连接器得到位置⽆关可执⾏程序。
--------------------------------------------------------
作者：12四象新玲资料集
链接：https://wenku.baidu.com/view/bf54bb2d7fd184254b35eefdc8d376eeaeaa1736.html
来源：百度文库
