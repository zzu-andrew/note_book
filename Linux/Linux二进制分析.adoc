...
- .o称为可重定位目标
包含二进制代码和数据，其形式可以和其他目标进行合并，创建一个可执行目标文件

我们通过因为.o文件也是ELF文件的一种，所以我么可以使用readelf -h 来查看.o文件的elf头数据

[source, bash]
----
andrew@andrew-G3-3590:/work/note_book/src/elf_parser$ readelf -h main.o
ELF 头：
...
放到
IDE_TASKFILE_STD_IN_FLAGSddd
IDE_TASKFILE_STD_IN_FLAGSdddd
d
d
...
ELF 头：
  ...
  Number of section headers:         14
  Section header string table index: 13
andrew@andrew-G3-3590:/work/note_book/src/elf_parser$ gcc  main.c
andrew@andrew-G3-3590:/work/note_book/src/elf_parser$ readelf -h a.out
ELF 头：
  Magic：   7f 45 4c 46 02 01 01 00 00 00 00 00 00 00 00 00
  类别:                              ELF64
  数据:                              2 补码，小端序 (little endian)
...
image::../image/image-2022-06-23-14-49-48-307.png[ELF接头]
...
[source, bash]
----
andrew@andrew-G3-3590:/work/note_book/src/elf_parser$ readelf -r a.out

重定位节 '.rela.dyn' at offset 0x528 contains 8 entries:
...
重定位节 '.rela.plt' at offset 0x5e8 contains 1 entry:
  偏移量          信息           类型           符号值        符号名称 + 加数
