...
#ifndef NODE_BOOKS_ELF_PARSER_H
#define NODE_BOOKS_ELF_PARSER_H
...
#define ERROR_EXIT(msg) do { \
    ...
    class ELF_reader {
    public:
        ELF_reader() : fd_(-1), program_length_(0), mmap_program_(nullptr) {}

        explicit ELF_reader(std::string file_path) : file_path_(std::move(file_path)) {
            load_memory_map();
        }
        ...
        void load_file(const std::string &path_name) {
